...
const SignUpPage = () => {
    ...
    const validateForm = () => {
        for (const key in formData) {
            if (!formData[key].trim()) return toast.error(`${key} is required`);
        }
                        ...
                        {Object.keys(formData).map((field, index) => (
                            <div className="form-control" key={index}>
                                <label className="label">
                                    <span className="label-text font-medium">{field.replace(/([A-Z])/g, ' $1').trim()}</span>
                                </label>
                                <input
                                    type={field === "password" ? (showPassword ? "text" : "password") : field === "birthday" ? "date" : "text"}
